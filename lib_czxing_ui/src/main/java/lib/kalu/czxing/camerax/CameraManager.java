package lib.kalu.czxing.camerax;
...
import lib.kalu.czxing.listener.OnCameraStatusChangeListener;
import lib.kalu.czxing.analyze.AnalyzerQrcode;
import lib.kalu.czxing.impl.ICameraImpl;
import lib.kalu.czxing.sensor.LightSensorEventManager;
import lib.kalu.czxing.util.BeepUtil;
import lib.kalu.czxing.util.LogUtil;
import lib.kalu.czxing.util.MathUtils;
import lib.kalu.czxing.util.VibratorUtil;
...
//        });
    ...
    public boolean analysis(@NonNull Activity activity, @Nullable ImageView imageView, @NonNull ImageProxy image) {
        ...
        BarcodeResult result = analyzerQrcode.analyzeImage(context, imageView, image, context.getResources().getConfiguration().orientation);
        if (null == result)
            return false;
    ...
    private boolean handleAutoZoom(@NonNull Activity activity, @Nullable BarcodeResult result, float distance) {
