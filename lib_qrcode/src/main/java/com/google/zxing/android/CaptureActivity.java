...
import com.google.zxing.android.decode.DecodeTool;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback {
    ...
    private QrcodeScanConfig.ZoomControllerLocation zoomControllerLocation;
    ...
    private void initIntent() {
        ...
        QrcodeScanConfig qrcodeScanConfig = (QrcodeScanConfig) intent.getSerializableExtra(QrcodeScanManager.INTENT_KEY_CONFIG_MODEL);


        String hintText = qrcodeScanConfig.getScanHintText();
        String scanColor = qrcodeScanConfig.getScanColor();
        boolean photoFlag = qrcodeScanConfig.isShowPhotoAlbum();
        beepFlag = qrcodeScanConfig.isShowBeep();
        vibrateFlag = qrcodeScanConfig.isShowVibrate();
        exitAnime = qrcodeScanConfig.getActivityExitAnime();
        zoomControllerFlag = qrcodeScanConfig.isShowZoomController();
        zoomControllerLocation = qrcodeScanConfig.getZoomControllerLocation();
...
//        intent.setAction(Intent.ACTION_GET_CONTENT);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1000) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    //需要压缩图片
                    Bitmap bitmapChoose = DecodeTool.decodeUriAsBitmap(CaptureActivity.this, uri);
    ...
    private void finishFailed(String errorMsg) {
        Intent intent = new Intent();
        intent.putExtra(QrcodeScanManager.INTENT_KEY_RESULT_ERROR, errorMsg);
        this.setResult(QrcodeScanManager.RESULT_FAIL, intent);
        this.finish();
        finishFinal();
    }

    private void finishCancle() {
        this.setResult(QrcodeScanManager.RESULT_CANCLE, null);
        finishFinal();
    }

    private void finishSuccess(String result) {
        Intent intent = new Intent();
        intent.putExtra(QrcodeScanManager.INTENT_KEY_RESULT_SUCCESS, result);
        this.setResult(QrcodeScanManager.RESULT_SUCCESS, intent);
        finishFinal();
    }
    ...
    private void updateZoomController() {
        ...
        if (zoomControllerFlag) {
            ...
            if (zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Left) {
                //垂直方向
            ...
            } else if (zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Right) {
                //垂直方向
                RelativeLayout.LayoutParams layoutParamsVertical = (RelativeLayout.LayoutParams) mLlRoomControllerVertical.getLayoutParams();
                layoutParamsVertical.height = framingRect.bottom - framingRect.top - size10 * 2;
                ...
                mLlRoomControllerVertical.setVisibility(View.VISIBLE);
            } else if (zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Bottom) {
                //横向
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_MOVE) {
            ...
            if (startY - moveY > 50) {
                if (zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Left
                        || zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Right) {
                    //垂直方向
                    //向上滑
                    zoomIn(1);
                }
                ...
                if (zoomControllerLocation == QrcodeScanConfig.ZoomControllerLocation.Bottom) {
                    //垂直方向
                    //向左滑
                    zoomOut(1);
                }
